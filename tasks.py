...
@celery.task()
def startPriceStreams( symbol, rediskeyname, price_state):
    # Update the bookTicker price of the trading symbols
    print("rediskeyname", rediskeyname)
    price_side = rediskeyname.split("-")

    print("price side", price_side)
    print("price_state",price_state)
    redis_price = price_side[0]
    redis_side = price_side[1]
    ...
    def handle_socket_message(price_data):
        ...
        if price_state =="ETOP" and float(price) <=float(redis_price):
            tpSlOrderPlacer()
                
        if price_state =="ELOW" and float(price) >=float(redis_price):
            tpSlOrderPlacer()
